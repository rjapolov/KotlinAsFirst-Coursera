...
fun isNumberHappy(number: Int): Boolean {
    fun sumInt(x: Int) = x / 10 + x % 10
    return sumInt(number / 100) == sumInt(number % 100)
}
...
fun circleInside(x1: Double, y1: Double, r1: Double,
                 x2: Double, y2: Double, r2: Double) = trackLength(x1, y1, x2, y2) + r1 <= r2
...
fun brickPasses(a: Int, b: Int, c: Int, r: Int, s: Int) =
        max(max(min(a, b), min(b, c)), min(a, c)) <= max(r, s) && (min(min(a, b), c)) <= min(r, c)
